...
export default function RoasterJsonLd({
  ...
  const structuredData: RoasterStructuredData = {
    ...
    "@type": "LocalBusiness",
    "@id": mainLocation
      ? `${process.env.NEXT_PUBLIC_APP_URL}/roasters/${roaster.id}/locations/${mainLocation.id}`
      : `${process.env.NEXT_PUBLIC_APP_URL}/roasters/${roaster.id}`,
    name: roaster.name,
    ...
    address: {
      "@type": "PostalAddress",
      streetAddress:
        mainLocation?.address || roaster.address || "Address not provided",
    ...
    sameAs: [roaster.website].filter(Boolean) as string[],
  };
