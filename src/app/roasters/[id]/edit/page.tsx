...
import { ArrowLeft, Save, Trash, MapPin, Phone, Globe } from "lucide-react";
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            ...
            throw new Error("You don't have permission to edit this roaster");
          } else {
            throw new Error("Failed to fetch roaster");
          }
        }

        const roaster = await response.json();
        ...
        setCurrentImageUrl(roaster.image || null);
        setCoffeeCount(roaster._count?.coffees || 0);
      } catch (err) {
